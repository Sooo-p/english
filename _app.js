...
const BLANK = {
  ...
  tipSeen:false, showGloss:true, showRo:true, micBlocked:false, scenesDone:{}
};
let S = load();
...
function micUsable(){ return !!SR && !S.micBlocked; }
...
function renderDone(){
  ...
  const more = el("button","primary", left ? "새 문장 5개 더 배우기" : "복습 더 하기");
  more.onclick = () => {
    if(left){ addBonus(5); startSession(false); } else startSession(true);
  };
  const sc = el("button","ghost","상황 연습 하러 가기");
  sc.onclick = () => { view = "scenes"; render(); };
  const home = el("button","ghost","홈으로");
  home.onclick = () => { view = "home"; render(); };
  $foot.append(more, sc, home);
  $body.appendChild(el("div","note","더 배우면 내일 복습도 그만큼 늘어납니다. 지금 내일 몫은 " + tomorrowCount() + "장이에요."));
}
