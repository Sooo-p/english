...
const BLANK = {
  ...
  load:"normal",                        /* 하루 분량: light | normal | heavy */
  spreadDate:null, spreadMoved:0, spreadDays:0,   /* 밀린 것 나눠 담기 기록 */
...
/* 하루 분량 — 하루를 걸러도 감당할 수 있는 크기를 유지하는 게 목적입니다 */
const LOADS = {
  light : { rev: 6,  name:"짧게",   note:"하루 3~5분" },
  normal: { rev: 12, name:"보통",   note:"하루 8~12분" },
  heavy : { rev: 25, name:"넉넉히", note:"하루 20분 이상" }
};
function loadCfg(){ return LOADS[S.load] || LOADS.normal; }
function REVIEW_CAP(){ return loadCfg().rev; }

/* 밀린 복습을 앞으로 며칠에 나눠 담습니다. 하루 한 번만 돕니다.
   며칠을 걸러도 "오늘 할 것"이 늘 한 세션 크기로 유지됩니다. */
function spreadDue(){
  const t = today();
  if(S.spreadDate === t) return;
  S.spreadDate = t;
  S.spreadMoved = 0; S.spreadDays = 0;

  const cap = REVIEW_CAP();
  /* 오늘까지 밀린 것 + 지난번에 우리가 뒤로 미뤄둔 것(sp 표시)을 함께 다시 배분합니다.
     그래야 분량을 '넉넉히'로 올렸을 때 미뤄둔 것이 도로 당겨집니다. */
  const pool = CARDS.filter(c => {
    const st = S.cards[c.id];
    return st && (st.due <= t || st.sp);
  });

  if(pool.length <= cap){
    pool.forEach(c => { S.cards[c.id].due = t; delete S.cards[c.id].sp; });
    save(); return;
  }

  shuffle(pool).forEach((c, i) => {
    const st = S.cards[c.id];
    if(i < cap){ st.due = t; delete st.sp; }
    else { st.due = addDays(1 + Math.floor((i - cap) / cap)); st.sp = true; }
  });
  S.spreadMoved = pool.length - cap;
  S.spreadDays = Math.ceil(S.spreadMoved / cap);
  save();
}

/* 마지막으로 공부한 날부터 며칠이 지났나 */
function daysMissed(){
  if(!S.streakDate) return 0;
  return Math.max(0, daysBetween(S.streakDate, today()) - 1);
}
...
function renderHome(){
  ...
  /* 며칠 걸렀을 때 — 밀린 것을 나눠 담았다고 알려줍니다 */
  const missed = daysMissed();
  if(S.spreadDate === today() && S.spreadMoved > 0){
    const box = el("div","tip");
    const t = el("div");
    t.innerHTML = (missed > 0 ? "<b>" + missed + "일 쉬셨네요. 괜찮습니다.</b><br>" : "<b>밀린 게 있었어요.</b><br>") +
      "복습 <b>" + S.spreadMoved + "장</b>을 앞으로 " + S.spreadDays + "일에 나눠 담았습니다. " +
      "오늘은 <b>" + waiting + "장</b>만 하시면 됩니다. 밀린 게 한꺼번에 쏟아지지 않게 해뒀어요.";
    box.appendChild(t);
...
function commit(ok){
  ...
  delete st.sp;                         /* 답했으니 '미뤄둔 것' 표시를 뗍니다 */
  S.cards[cur.c.id] = st;
  save();
}
...
function syncLoad(){
  const cfg = loadCfg();
  document.querySelectorAll("#segLoad button").forEach(b => {
    b.setAttribute("aria-pressed", String(b.dataset.v === (S.load || "normal")));
  });
  const dueNow = dueCards().length;
  document.getElementById("loadNote").textContent =
    cfg.name + " — 복습을 하루 최대 " + cfg.rev + "장까지만 내보냅니다. " + cfg.note + "." +
    (dueNow > cfg.rev ? " 지금 밀린 " + dueNow + "장은 다음 날들로 나눠 담깁니다." : "");
}
document.querySelectorAll("#segLoad button").forEach(b => {
  b.onclick = () => {
    S.load = b.dataset.v;
    S.spreadDate = null;          /* 분량이 바뀌었으니 다시 나눠 담습니다 */
    save(); spreadDue(); syncLoad();
    msg("하루 분량을 '" + loadCfg().name + "'으로 바꿨어요. 닫으면 새 분량으로 다시 짭니다.");
  };
});
...
document.getElementById("gear").onclick = () => {
  ...
  syncToggles(); syncLoad(); msg(""); $sheet.hidden = false;
};
